...
//Function to update the config register in normal mode
...
//This function tells if measurement is running now 
BME280_Status_t IsMeasuring(BME280_Device_t *Device, uint8_t *IsMeasuringPtr)
{
    ...
    if (StatusBit)
    {
        *IsMeasuringPtr = 1; //1 when conversion is running
    } 
    else 
    {
        *IsMeasuringPtr = 0; // 0 when results transferred to data registers
    } 

    return BME280_OK; // Return success
}

//This function tells if non volatile memory is being copied right now
//The data are copied at every power-on-reset and before every conversion
BME280_Status_t IsNVMCopying(BME280_Device_t *Device, uint8_t *IsCopyingPtr)
{
    ...
    if (StatusBit)
    {
        *IsCopyingPtr = 1; //1 when NVM is being copied to image registers
    } 
    else 
    {
        *IsCopyingPtr = 0; // 0 when copying is done
    } 
...
BME280_Status_t GetMeasurements(BME280_Device_t *Device, OutputData_t *OutputDataPtr)
{
    ...
    //Skip this function if every oversampling is set to skip
    if (Device->osrs_t == BME280_OSRS_SKIP && Device->osrs_p == BME280_OSRS_SKIP && Device->osrs_h == BME280_OSRS_SKIP)
    {
        return BME280_OK;
    }

    //Cannot get pressure or humidity measurements without measuring temperature 
    if((Device->osrs_t == BME280_OSRS_SKIP) && (Device->osrs_p != BME280_OSRS_SKIP || Device->osrs_h != BME280_OSRS_SKIP))
    {
    ...
    Status = ReadRawData(Device, &RawData); //Reading raw data 
...
//interface must be initialized before calling this function, and pointer to interface must be set in Device->InterfacePtr
